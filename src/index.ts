...
// Store active session handlers (in-memory, will be reconstructed on worker restart)
...
app.get('/github.com/:owner/:repo{.*}', async (c) => {
  const owner = c.req.param('owner');
  const repoPath = c.req.param('repo');
  ...
  try {
    ...
    if (existingSessionId) {
      ...
      try {
        ...
        // Session is dead, continue to create new one
...
async function initializeSession(
  ...
  try {
    ...
    await sandbox.exec(`nohup sh -c '${startCmd}' > /tmp/app.log 2>&1 &`);
    ...
    if (analysis.environment.previewable && analysis.environment.port) {
      try {
        const portInfo = await sandbox.exposePort(analysis.environment.port);
