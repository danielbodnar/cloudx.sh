...
export class SessionHandler {
  ...
  private sandbox: DurableObjectStub<Sandbox>;
  ...
  private async startApplication(): Promise<void> {
    ...
    if (this.environment.previewable && this.environment.port) {
      try {
        const hostname = new URL(this.env.GITHUB_API_BASE || 'https://cloudx.sh').hostname;
        const previewInfo = await this.sandbox.exposePort(this.environment.port, { hostname });
